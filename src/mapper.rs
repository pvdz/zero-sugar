...
impl<'a> Mapper<'a> {
    ...
    pub fn map_statement(&self, mut stmt: Statement<'a>) -> Statement<'a> {
        ...
        // If any after visitor requested a revisit, map the statement again
        if should_revisit {
            stmt = self.map_statement(stmt);
        }
    ...
    fn map_expression(&self, mut expr: Expression<'a>) -> Expression<'a> {
        ...
        // If any after visitor requested a revisit, map the expression again
        if should_revisit {
